...
#   binding magnitude. Exp 2 uses 50% congruent / 50% incongruent test trials.
#   Finding: cond.type main effect significant (faster RT in IC than IP);
#            validity did NOT reach significance at 50/50 split.
#
...
message("✓ n = ", n_distinct(dat_exp2$participant), " participants.")
...
p_rt <- ggplot(rt_summary,
  ...
  labs(x = "Block", y = "Mean log(RT) in ms",
       ...
       title = "Exp 2: Reaction Time — IC vs IP") +
  theme_classic(base_size = 13) +
  theme(legend.position = "top")
p_rt
ggsave(here("figures", "exp2_RT_IC_vs_IP.png"), p_rt, width = 8, height = 6, dpi = 300)
