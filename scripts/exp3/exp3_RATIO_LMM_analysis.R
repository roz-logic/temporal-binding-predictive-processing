# =============================================================================
# Experiment 3: Temporal Binding — RATIO Analysis
# =============================================================================
# Author      : Oz
...
#               for Experiment 3 (n = 41; 3 excluded from 44 for non-completion).
#               Four analyses:
...
#
# Key design differences from Exp 2:
#   - Test phase: 80% congruent / 20% incongruent (was 50% / 50%)
#   - Sample: n = 41 (3 excluded from 44, not 1 from 42)
#   - All other parameters identical to Exp 2:
#       Fixed IEI: 650 ms | Random IEI: 150–1150 ms
#       Acquisition: 30 trials + 20% catch | Test: 50 trials
#
# Rationale for Exp 3:
#   The 50/50 congruency split in Exp 2 may have been insufficient to build a
#   strong enough identity prediction signal. Exp 3 increases congruency to
#   80% to maximise any potential validity effect on RT and binding.
#
# DV note:
#   MC, CP    : log_RATIO — cross-experiment analyses (Exp 3a vs 3b); TC data require log-transform
#   mvsp      : RATIO (raw) — within Exp 3a (IC vs IP); normality acceptable
#   TC        : log(RATIO)  — Exp 3b TC blocks; normality required log-transform
#
...
#
# Data file to use: rename Exp3_data(41).csv → exp3_data_n41.csv in repo
# =============================================================================
...
# --- 2.2 Sanity checks -------------------------------------------------------
stopifnot(sum(is.na(dat_exp3$RATIO)) == 0)
stopifnot(n_distinct(dat_exp3$participant) == 41)
message("✓ n = ", n_distinct(dat_exp3$participant), " participants after exclusion.")

print(contrasts(dat_exp3$cond.type))
print(contrasts(dat_exp3$acq.time))
...
dat_tc   <- dat_exp3 %>% filter(cond.type == "TC")
...
# DV: log_RATIO  (cross-experiment: Exp 3a IC vs Exp 3b TC — TC data require log-transform)
# =============================================================================

message("\n--- Analysis MC: Identity Control vs Temporal Control ---")

mc_null <- lmer(log_RATIO ~ 1 + (1 + cond.type + acq.time | participant),
...
mc_full <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type + acq.time | participant),
                dat_mc, REML = FALSE, na.action = na.exclude)
...
mc_rand1 <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type | participant),
                 dat_mc, REML = TRUE, na.action = na.exclude)
mc_rand2 <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type + acq.time | participant),
                 dat_mc, REML = TRUE, na.action = na.exclude)
...
mc_afex <- mixed(
  log_RATIO ~ cond.type * acq.time + (1 + cond.type + acq.time | participant),
...
plot(fitted(mc_full), resid(mc_full),
     xlab = "Fitted", ylab = "Residuals", main = "MC Exp 3: Fitted vs Residuals")

# =============================================================================
# SECTION 6: ANALYSIS CP — Identity Prediction vs Temporal Control [2 × 2]
# DV: log_RATIO  (cross-experiment: Exp 3a IP vs Exp 3b TC — TC data require log-transform)
# =============================================================================

message("\n--- Analysis CP: Identity Prediction vs Temporal Control ---")

cp_null <- lmer(log_RATIO ~ 1 + (1 + cond.type + acq.time | participant),
...
# Backward
cp_full <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type + acq.time | participant),
                dat_cp, REML = FALSE, na.action = na.exclude)
...
# Random effects
cp_rand1 <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type | participant),
                 dat_cp, REML = TRUE, na.action = na.exclude)
cp_rand2 <- lmer(log_RATIO ~ cond.type * acq.time + (1 + cond.type + acq.time | participant),
                 dat_cp, REML = TRUE, na.action = na.exclude)
...
plot(fitted(cp_full), resid(cp_full),
     xlab = "Fitted", ylab = "Residuals", main = "CP Exp 3: Fitted vs Residuals")

# =============================================================================
# SECTION 7: ANALYSIS mvsp — IC vs IP [2 × 2 × 2]
# DV: RATIO
# Predictors: cond.type × acq.time × is_valid
# Note: 80% congruent / 20% incongruent in Exp 3 — stronger validity signal
#       expected vs Exp 2 (50/50). Three-way interaction probed below.
# =============================================================================
...
plot(fitted(mvsp_full), resid(mvsp_full),
     xlab = "Fitted", ylab = "Residuals", main = "mvsp Exp 3: Fitted vs Residuals")
...
plot(fitted(tc_full), resid(tc_full),
     xlab = "Fitted", ylab = "Residuals", main = "TC Exp 3: Fitted vs Residuals")
...
p_mc <- ggplot(mc_summary,
  ...
  labs(x = "Block", y = "Mean RATIO",
       ...
       title = "Exp 3: Identity Control vs Temporal Control") +
  theme_classic(base_size = 13) +
  theme(legend.position = "top")
p_mc
ggsave(here("figures", "exp3_MC_RATIO.png"), p_mc, width = 8, height = 6, dpi = 300)
...
p_cp <- ggplot(cp_summary,
  ...
  labs(x = "Block", y = "Mean RATIO",
       fill = "Acquisition Timing",
       title = "Exp 3: Identity Prediction vs Temporal Control") +
  theme_classic(base_size = 13) +
  theme(legend.position = "top")
p_cp
ggsave(here("figures", "exp3_CP_RATIO.png"), p_cp, width = 8, height = 6, dpi = 300)
...
p_mvsp <- ggplot(mvsp_summary,
  ...
  labs(x = "Block", y = "Mean RATIO",
       ...
       title = "Exp 3: IC vs IP — Validity Effect (80% congruent)") +
  theme_classic(base_size = 13) +
  theme(legend.position = "top")
p_mvsp
ggsave(here("figures", "exp3_mvsp_RATIO.png"), p_mvsp, width = 8, height = 6, dpi = 300)
...
p_tc <- ggplot(tc_summary,
  ...
  labs(x = "Block (Acq | Test)", y = "Mean log(RATIO)",
       ...
       title = "Exp 3: Temporal Control — log(RATIO)") +
  theme_classic(base_size = 13) +
  theme(legend.position = "top")
p_tc
ggsave(here("figures", "exp3_TC_logRATIO.png"), p_tc, width = 8, height = 6, dpi = 300)
